...
type
  ...
  public
    constructor Create(ADocument: TMFSchemeXml); virtual;
    constructor CreateName(ADocument: TMFSchemeXml; const AName: string); virtual;
    constructor CreateNameValue(ADocument: TMFSchemeXml; const AName, AValue: string); virtual;
    constructor CreateType(ADocument: TMFSchemeXml; AType: TXmlElementType); virtual;
    ...
    function ReadWidestring(const AName: string; const ADefault: widestring = ''): widestring; virtual;
    ...
    procedure WriteWidestring(const AName: string; const AValue: widestring; const ADefault: widestring = ''); virtual;
    ...
    property Document: TMFSchemeXml read FDocument write FDocument;
  ...
  TMFSchemeXml = class(TPersistent)
  private
  ...
  protected
    procedure CopyFrom(Source: TMFSchemeXml); virtual;
...
const
  ...
  cControlChars: set of char = [#9, #10, #13, #32]; 
...
begin
  ...
  end else if Source is TMFSchemeXml then begin
    Assign(TMFSchemeXml(Source).FRootNodes);
  end else
    inherited;
end;
...
constructor TXmlNode.Create(ADocument: TMFSchemeXml);
begin
  inherited Create;
  FDocument := ADocument;
end;

constructor TXmlNode.CreateName(ADocument: TMFSchemeXml;
  const AName: string);
begin
  Create(ADocument);
  Name := AName;
...
constructor TXmlNode.CreateNameValue(ADocument: TMFSchemeXml; const AName,
  AValue: string);
begin
  Create(ADocument);
  Name := AName;
...
constructor TXmlNode.CreateType(ADocument: TMFSchemeXml;
...
procedure TMFSchemeXml.Assign(Source: TPersistent);

procedure SetDocumentRecursively(ANode: TXmlNode; ADocument: TMFSchemeXml);
var
  i: integer;
begin
...
begin
  if Source is TMFSchemeXml then begin
    
    FBinaryEncoding := TMFSchemeXml(Source).FBinaryEncoding;
    FDropCommentsOnParse := TMFSchemeXml(Source).FDropCommentsOnParse;
    FExternalEncoding := TMFSchemeXml(Source).FExternalEncoding;
    FParserWarnings := TMFSchemeXml(Source).FParserWarnings;
    FIndentString := TMFSchemeXml(Source).FIndentString;
    FUseFullNodes := TMFSchemeXml(Source).FUseFullNodes;
    FUtf8Encoded := TMFSchemeXml(Source).FUtf8Encoded;
    FWriteOnDefault := TMFSchemeXml(Source).FWriteOnDefault;
    FXmlFormat := TMFSchemeXml(Source).FXmlFormat;
    FSortAttributes := TMFSchemeXml(Source).FSortAttributes;
    
    FRootNodes.Assign(TMFSchemeXml(Source).FRootNodes);
...
procedure TMFSchemeXml.CopyFrom(Source: TMFSchemeXml);
begin
  if not assigned(Source) then exit;
  Assign(Source);
end;

constructor TMFSchemeXml.Create;
begin
  inherited Create;
...
constructor TMFSchemeXml.CreateName(const ARootName: string);
begin
  Create;
  Root.Name := ARootName;
end;

destructor TMFSchemeXml.Destroy;
begin
  FreeAndNil(FRootNodes);
  inherited;
end;

procedure TMFSchemeXml.DoNodeLoaded(Node: TXmlNode);
begin
  if assigned(FOnNodeLoaded) then
    FOnNodeLoaded(Self, Node);
end;

procedure TMFSchemeXml.DoNodeNew(Node: TXmlNode);
begin
  if assigned(FOnNodeNew) then
    FOnNodeNew(Self, Node);
end;

procedure TMFSchemeXml.DoProgress(Size: integer);
begin
  if assigned(FOnProgress) then FOnProgress(Self, Size);
end;

procedure TMFSchemeXml.DoUnicodeLoss(Sender: TObject);
begin
  if assigned(FOnUnicodeLoss) then FOnUnicodeLoss(Self);
end;

function TMFSchemeXml.GetCommentString: string;
...
function TMFSchemeXml.GetEncodingString: string;
begin
  Result := '';
...
function TMFSchemeXml.GetEntityByName(AName: string): string;
var
  i, j: integer;
begin
  Result := '';
...
function TMFSchemeXml.GetRoot: TXmlNode;
begin
  Result := FRootNodes.NodeByElementType(xeNormal);
end;

function TMFSchemeXml.GetStyleSheetString: string;
var
  ANode: TXmlNode;
begin
  Result := '';
...
function TMFSchemeXml.GetVersionString: string;
begin
  Result := '';
  if (FRootNodes.NodeCount > 0) and (FRootNodes[0].ElementType = xeDeclaration) then
...
function TMFSchemeXml.IsEmpty: boolean;
var
  ARoot: TXmlNode;
begin
  Result := True;
...
function TMFSchemeXml.LineFeed: string;
begin
  case XmlFormat of
  xfReadable: Result := #13#10;
  xfCompact:  Result := #10;
...
procedure TMFSchemeXml.LoadFromFile(const FileName: string);
var
  S: TStream;
begin
...
procedure TMFSchemeXml.LoadFromStream(Stream: TStream);
var
...
procedure TMFSchemeXml.ParseDTD(ANode: TXmlNode; S: TStream);
...
procedure TMFSchemeXml.ReadFromStream(S: TStream);
var
  i: integer;
  ANode: TXmlNode;
  AEncoding: string;
...
procedure TMFSchemeXml.ReadFromString(const AValue: string);
var
  S: TStream;
begin
  S := TStringStream.Create(AValue);
...
procedure TMFSchemeXml.ResolveEntityReferences;
begin
  if assigned(Root) then
    Root.ResolveEntityReferences;
end;

procedure TMFSchemeXml.SaveToFile(const FileName: string);
var
  S: TStream;
begin
...
procedure TMFSchemeXml.SaveToStream(Stream: TStream);
var
...
procedure TMFSchemeXml.SetCommentString(const Value: string);
...
procedure TMFSchemeXml.SetDefaults;
begin
...
procedure TMFSchemeXml.SetEncodingString(const Value: string);
var
  ANode: TXmlNode;
begin
...
procedure TMFSchemeXml.SetStyleSheetString(const Value: string);
var
  ANode: TXmlNode;
begin
...
procedure TMFSchemeXml.SetVersionString(const Value: string);
var
  ANode: TXmlNode;
begin
...
procedure TMFSchemeXml.WriteToStream(S: TStream);
var
  i: integer;
begin
...
function TMFSchemeXml.WriteToString: string;
var
  S: TStringStream;
begin
  S := TStringStream.Create('');
